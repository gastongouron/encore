import LocalizedStrings from 'react-localization';

const translations = {
 fr: {
   homeHello:"La communauté qui partage ses expériences de concert et vous fait découvrir les meilleurs artistes sur scène.",
   baseline:"les concerts devaient finir avec ",
   contributor: "Contributeurs les plus actifs",
   artistsHome: "Populaires en ce moment",
   sorry: "Désolé, on ne le trouve pas!",
   feature1Bold:"Donne et consulte",
   feature1Light1:" des avis",
   feature1Light2:" sur des shows du moment ",
   feature1Light3:"et les artistes à découvrir",
   feature2Bold1:"Partage les temps forts du",
   feature2Bold2:" concert en images",
   feature2Light1:" avec les plus ",   
   feature2Light2:"belles photos et vidéos",
   feature3Bold:"Découvre des artistes",
   feature3Light1:" grâce aux contenus de la ",
   feature3Light2:"communauté",
   lorem: "Nous avons créée cette plateforme, pour les artistes et leur musique  pour les fans, les groupies du premier rang, pour ceux qui tapent du pied au fond de la salle, pour tout ceux qui assistent à un concert pour vivre un moment unique.",
   lorem2: "Pour vous et nous quoi!",
   lorem3: "Pour la musique",
   lorem4: "Pour les émotions qu'elle nous procure depuis toujours",   
   lorem5: "Pour les artistes",
   lorem6: "Pour le créativité et le sensibilité qu'ile partagent avec nous",
   lorem6bis: "Pour la générosité et l'égergie qu'ils donnent sur scène, sans limite",
   lorem7: "Pour nous tous",   
   lorem8: "Pour la groupie du premier range, pour celui qui tape du pied au fond de la salle",
   lorem8bis: "Pour tout ceux qui vont à des concerts pour vivre des moments uniques",
   community: "Découvrez les meilleurs artistes du moment recommandés par la communauté encore",
   getStarted:"nous rejoindre maintenant",
   login:"Se connecter",
   mustLogin:"Veuillez vous identifier ou vous inscrire",
   facebook:"Avec Facebook",
   signup:"Inscription",
   sign:"S'inscrire",
   firstName:"Prénom",
   lastName:"Nom",
   required:"Requis",
   invalidEmail:"Format d'adresse email invalide",
   resetPassword:"Mot de passe oublié",
   passwordConfirmation:"Confirmation du mot de passe",
   resetting:"Réinitialization du mot de passe...",
   reset:"Réinitialiser mot de passe",
   avatar:"Avatar",
   logout:"Se déconnecter",
   profile:"Profil",
   email:"Adresse email",
   password:"Mot de passe",
   logging:"Un instant...",
   request:"Renouveller le mot de passe",
   requesting:"Un instant...",
   failed:"Mot de passe ou identifiant invdalide",
   requestResetPasswordSucceeded:"Votre requête de renouvellement de mot de passe à bien été prise en compte. Vous devriez recevoir un email contenant un lien de renouvellement très prochainement.",
   artists:"Artistes",
   switch:"Changer de langue",
   welcome:"Bienvenue!",
   reviews:'Profil de {username}',
   enrich:"Aidez les membres de la communauté à se projetter dans le show de {artistname}.",
   review:"Votre avis à propos de {name}",  
   reviewHint:"Quelle expérience incroyable, c'était vraiment cool, j'ai passé une trop bonne soirée, merci à tout les membres de {artistname}!", 
   reviewBodyLabel:"Donnez tout!", 
   wanna:"Vous aimeriez donner votre avis?",
   search:"Rechercher un artiste ou un genre...",
   userSearch: "Rechercher un contributeur...",
   new:"Donner mon avis",
   cancel:"Annuler",
   save:"Publier",
   edit:"Éditer",
   update:"Valider",
   delete:"Supprimer",
   deleteMedia:"Supprimer ce média",
   addMedia:"Télécharger",
   changeMedia:"Changer ce média",
   created_at:"Créé ",
   updated_at:"Mis à jour ",
   justNow: "À l'instant",
   reviewLabel: "Expérience",
   reviewsLabel: "Expériences",
   beTheFirst: "Partagez votre expérience!",
   readMore: " voir plus.",
   readLess: " voir moins.",
   follow: "s'abonner",
   unfollow: "se désabonner",
   following: "abonnement",
   followings: "abonnements",
   follower: "abonné",
   followers: "abonnés",

   followedBy: " vous suis désormais",
   unfollowedBy: " à cessé de vous suivre",
   recommend: "vient de partager une expérience à propos de ",
   male: "Mr.",
   female: "Mme.",
   birthDate: "Date de naissance",
   tos: "J'accepte les conditions d'utilisation",
   people: "personnes",
   person: "personne",
   cookie: {
      accept: "J'accepte",
      text: "Nous vous informons que ce site utilise des cookies.",
   },
   policy: {
      link:"Confidentialité",
      title:"Encore: Politique de confidentialité",
      p1:"Votre vie privée est importante pour nous. Encore a pour politique de respecter votre vie privée à l’égard des informations que nous pouvons collecter sur votre site internet. ",
      p2:"Nous ne vous demandons pas vos informations personnelles sauf si nous en avons vraiment besoin. Lorsque nous le faisons, nous ne collectons ce dont nous avons besoin que par des moyens équitables et licites et, le cas échéant, à votre connaissance ou avec votre consentement. Nous allons également vous faire savoir pourquoi nous le recueillons et comment il sera utilisé",
      p3:"Nous ne partageons pas vos informations personnelles avec des tiers, sauf si requis par la loi. Nous ne conserverons ces informations que le temps nécessaire pour vous fournir un service",
      p4:"Nous ne stockons pas vos informations personnelles sur nos serveurs, sauf si cela est nécessaire pour vous fournir un service. Ce que nous stockons, nous le protégerons par des moyens commercialement acceptables afin de protéger vos informations personnelles contre la perte ou le vol, ainsi que contre les attaques non autorisées. accès, divulgation, copie, utilisation ou modification.!)",
   },
   notFound: "Désolé, impossible de mettre la main sur ce que vous cherchez...",
   step1Title: "Notez le concert de 1 à 5",
   step1Subtitle: "PS: si on vous à payé un verre ça compte pas pour la générosité",
   step2Title: "Alors, c'était comment?",
   step2Subtitle: "La musique c'est une question d'émotions",
   step3Title: "Ajoutez une photo / vidéo (facultatif)",
   step3Subtitle: "Aidez les membres de la communauté à se projeter dans le show",

   performance: "Performance",
   generosity: "Générosité",
   technics: "Son",
   ambiant: "Ambiance",
   noNotifications: "Bienvenue sur encore!",
   noNotificationsP1: "Vous recevez une notification lorsque une personne que vous suivez donne son avis à propos d'un artiste ainsi que lors de l'acquisition d'un nouveau follower!",
   picture: "photo de profil",
   hot: "Hot",
   next: "Suivant",
   back: "Précédent",
   reviewLengthError: "Votre expérience doit compter 20 caractères minimum",
 },
 en:{
   homeHello:"A community sharing experiences about concerts to figure out who's the best on stage",
   baseline:"every show should end with an",
   contributor: "Top contributors",
   artistsHome: "Most popular artists",
   sorry: "Sorry, we can't find that person!",
   feature1Bold:"Give and consult",
   feature1Light1:" reviews",
   feature1Light2:" about current shows ",
   feature1Light3:"and artists to discover",
   feature2Bold1:"Share the highlights of the",
   feature2Bold2:" concert in pictures",
   feature2Light1:" with ",
   feature2Light2:"your bests photos and videos",
   feature3Bold:"Discover artists",
   feature3Light1:" thanks to the contents ",
   feature3Light2:"of the community",
   lorem: "We created this platform for artists and their music for fans, groupies of the first rank, for those who stomp in the back of the room, for all those who attend a concert to live a unique moment.",
   lorem2: "Just for all of us!",
   lorem3: "We do it for music",
   lorem4: "For the emotions it brings to all of us, through all ages",   
   lorem5: "We do it for artists",
   lorem6: "For the creativity and sensitivyty the put in their art",
   lorem6bis: "For generosity and energy they give us on stage with no limit",
   lorem7: "We do it for all of us",   
   lorem8: "For the front row groupie, for the one who stamps in the back",
   lorem8bis: "For all those who attend a concert to live unique moments",
   community: "Discover the best artists of the moment recommended by the community",
   getStarted:"Join us now",
   login:"Login",
   mustLogin:"Please log in or sign up",
   facebook:"with Facebook",
   signup:"Sign up",
   sign:"Sign up",
   firstName:"First name",
   lastName:"Last name",
   required:"Required",
   invalidEmail:"Invalid email format",
   resetPassword:"Forgotten password",
   passwordConfirmation:"Password confirmation",
   resetting:"Resetting password...",
   reset:"Reset password",
   avatar:"Avatar",
   logout:"Logout",
   profile:"Profile",
   email:"Email",
   password:"Password",
   logging:"Logging in...",
   request:"Request password reset",
   requesting:"Requesting...",
   failed:"Invalid credentials",
   requestResetPasswordSucceeded:"Your request to reset you password was received. You should receive an email shortly with a link to reset your password.",
   artists:"Artists",
   switch:"Switch language",
   welcome:"Welcome!",
   reviews:"{username}'s profile",
   review:"Your review for {name}",   
   enrich:"Help the community with a preview of {artistname}'s show.",
   reviewHint:"It was amazing, I had a great time, {artistname}'s show was truly wonderful!", 
   reviewBodyLabel:"Give everything you've got!", 
   wanna:"Wanna leave a review?",
   search:"Artist or genre search...",
   userSearch: "Contributor search...",
   new:"New review",
   cancel:"Cancel",
   save:"Publish",
   edit:"Edit",
   update:"Update",
   delete:"Delete",
   deleteMedia:"Delete media file",
   addMedia:"Download",
   changeMedia:"Change media",
   created_at:"Created ",
   updated_at:"Updated ",
   justNow: "Just now",
   reviewLabel: "Experience",
   reviewsLabel: "Experiences",
   beTheFirst: "Share your experience!",
   readMore: " read more.",
   readLess: " read less.",
   follow: "follow",
   unfollow: "unfollow",
   following: "person followed",
   followings: "people followed",
   follower: "follower",
   followers: "followers",
   followedBy: " is now following you",
   unfollowedBy: " has stopped following you",
   recommend: "shared an experience about ",
   people: "people",
   person: "person",
   male: "Mr.",
   female: "Mrs.",
   birthDate: "Birth date",
   tos: "I accept terms of service",
   cookie: {
      accept: "I accept",
      text: "We hereby inform you that we use cookies.",
   },
   policy: {
      link:"Privacy policy",
      title:"Encore: Privacy Policy",
      p1:"Your privacy is important to us. It is Encore's policy to respect your privacy regarding any information we may collect from you across our website.",
      p2:"We don’t ask for your personal information unless we truly need it. When we do, we’ll only collect what we need by fair and lawful means and, where appropriate, with your knowledge or consent. We’ll also let you know why we’re collecting it and how it will be used",
      p3:"We don’t share your personal information with third-parties, except where required by law. We will only retain personal information for as long as necessary to provide you with a service",
      p4:"We don’t store your personal information on our servers unless it’s required for providing a service to you. What we store, we’ll protect within commercially acceptable means to protect your personal information from loss or theft, as well as unauthorized access, disclosure, copying, use or modification.!)",
   },
   notFound: "Sorry, we can't find what you're looking for...",
   step1Title: "Rate the show from 1 to 5",
   step1Subtitle: "If someone got you a drink it's not supposed to increase generosity score!",
   step2Title: "So, how was it?",
   step2Subtitle: "Music is all about feelings",
   step3Title: "Add a picture / video (not mendatory)",
   step3Subtitle: "Help the community with visual insights from the show",

   performance: "Performance",
   generosity: "Generosity",
   technics: "Sound",
   ambiant: "Ambiant",
   noNotifications: "You haven't been notified yet!",
   noNotificationsP1: "You get notified when someone you follow shares an experience or when someone follows you!",
   picture: "profile picture",
   hot: "Hot",
   next: "Next",
   back: "Back",
   reviewLengthError: "Must be at least 20 characters",
 }
}
const strings = new LocalizedStrings(translations);

export default strings
